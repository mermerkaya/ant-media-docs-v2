...
```mermaid
flowchart TD
    AMS[Ant Media Server] -->|FCM key| FCM[Firebase Cloud Messaging]
    AMS -->|APNs key| APNs[Apple Push Notification service]
    FCM -->|notification| Android[Android Device]
    APNs -->|notification| iOS[iOS Device]
    Client[Web/Mobile Client] -->|registerPushNotificationToken| AMS
    Client -->|sendPushNotification| AMS
```
