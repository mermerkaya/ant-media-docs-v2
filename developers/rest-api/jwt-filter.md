...
sequenceDiagram
    ...
    Client->>Client: Generate JWT with HS256 + secret key
    Client->>AMS: POST /broadcasts/create\nAuthorization: Bearer {JWT}
    AMS->>AMS: Validate JWT signature
    AMS-->>Client: 200 OK (valid token)
    AMS-->>Client: 401 Unauthorized (invalid/expired token)
```
