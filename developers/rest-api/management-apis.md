...
```mermaid
flowchart TD
    A[Client] -->|Choose method| B{Auth method}
    B -->|JWT| C[Generate JWT with secret key]
    B -->|Username/Password| D[POST /users/authenticate]
    C -->|ProxyAuthorization header| E[Management API]
    D -->|JSESSIONID cookie| E
    E --> F[Applications, Users, System Resources]
```
