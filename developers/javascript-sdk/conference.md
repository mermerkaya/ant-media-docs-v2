...
```mermaid
flowchart TD
    P1[Participant 1] -->|publish stream1| AMS[Ant Media Server Room]
    P2[Participant 2] -->|publish stream2| AMS
    P3[Participant 3] -->|publish stream3| AMS
    AMS -->|play all tracks| P1
    AMS -->|play all tracks| P2
    AMS -->|play all tracks| P3
```
...
1. Create a new file named `conference.html`.
...
4. Open the page in your browser at: `http://localhost:8000/conference.html`
...
Format: `protocol://IP_ADDRESS:PORT/APPLICATION_NAME/websocket`
...
The callback receives messages and notifications from the server. The `newTrackAvailable` event is triggered when new remote streams are available in the room.
