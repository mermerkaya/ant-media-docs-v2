...
sidebar_position: 2
description: Ant Media Server supports creating application development on the fly. You may create separate applications for each stream domains if you are managing multiple streams from same server.
...
```mermaid
flowchart TD
    A[Need a new AMS application] --> B{Preferred method?}
    B -->|Web UI| C[Dashboard → New Application]
    B -->|Shell| D[create_app.sh script]
    B -->|API| E[REST API POST /applications]
    C & D & E --> F[Application created]
    F --> G[Restart AMS if using shell method]
```
