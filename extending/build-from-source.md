...
```mermaid
flowchart TD
    A[Clone ant-media-server-parent] --> B[mvn install parent]
    B --> C{Edition?}
    C -->|Community| D[Build Web Panel]
    C -->|Enterprise| E[Build Enterprise Source]
    D --> F[Clone AMS repo]
    E --> F
    F --> G[mvn clean install]
    G --> H[Run repackage script]
    H --> I[ant-media-server-x.x.x.zip in target/]
```
