...
The plugin architecture is versatile enough to address various developer needs. For example, the [MCU](./available-plugins.md#filter-plugin) (Merge/Mix Streams) and [DRM](./available-plugins.md#drm-plugin) features was developed by the Ant Media team as a plugin, showcasing the wide range of possible use cases.

```mermaid
flowchart TD
    A[Plugin JAR file] -->|copy to /plugins| B[AMS Plugins Directory]
    B -->|chown antmedia:antmedia| C[Set ownership]
    C -->|service antmedia restart| D[AMS loads plugin]
    D -->|grep plugin in logs| E[Verify loaded]
```
...
* **Plugin Architecture:** To understand how the plugin system works under the hood, read our [Plugin Architecture page](./architecture.md).

* **Developing Your First Plugin:** Ready to build your own custom functionality? Follow our [Plugin Development Tutorial](./developing.md).
