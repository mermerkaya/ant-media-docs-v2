...
**Recommended Reading:**
- Review [Plugin Architecture](./architecture.md) to understand architectural patterns and use cases
...
For architectural patterns and data flow diagrams, see the [Plugin Architecture](./architecture.md) guide.
...
sequenceDiagram
    ...
    AMS->>Plugin: setApplicationContext()
    Plugin->>AMS: addFrameListener(streamId, listener)
    AMS->>Listener: setVideoStreamInfo()
    AMS->>Listener: onVideoFrame() [per frame]
    AMS->>Listener: writeTrailer() [stream ends]
    Plugin->>AMS: removeFrameListener()
```
...
- **Return same frame:** Pass unmodified data to next stage ([Asynchronous pattern](./architecture.md#asynchronous-case))
- **Return modified frame:** Pass your manipulated data forward ([Synchronous pattern](./architecture.md#synchronous-case))
- **Return null:** Stop frame propagation, plugin is final destination ([Last Point pattern](./architecture.md#last-point-case))

See [Plugin Architecture - Use Cases](./architecture.md#use-cases) for more details.
...
For the **[First Point](./architecture.md#first-point-case)** use case (plugin as stream source), use Custom Broadcast to inject streams into AMS from external sources.
...
- **[Filter Plugin](./available-plugins.md):** Real-time audio/video manipulation, MCU conferencing
- **[TensorFlow Plugin](./available-plugins.md):** AI/ML integration patterns
