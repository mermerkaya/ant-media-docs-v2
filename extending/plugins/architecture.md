...
In the most basic explanation, a plug-in is added into the regular flow of AMS in ways that we will examine. Once you understand the structure and use cases, you can proceed to [building your own plugin](./developing.md).

```mermaid
flowchart LR
    Ingest[Stream Ingest RTMP/WebRTC/SRT] --> Decoder[Decoder]
    Decoder --> FL[IFrameListener Plugins]
    Ingest --> PL[IPacketListener Plugins]
    FL --> Encoder[Encoder]
    PL --> Encoder
    Encoder --> Output[HLS / WebRTC / DASH Output]
```
...
Ant Media Server already implements many protocols like WebRTC, RTMP, SRT and stream source pulling. But you may want to implement other streaming protocols for publishing to your server. In this case, you can create a [CustomBroadcast](./developing.md#custom-broadcast-api) and feed it from your plugin.

For example, you may have a plugin that receives frames from an external source and feeds them to the [CustomBroadcast](./developing.md#custom-broadcast-api). Then [CustomBroadcast](./developing.md#custom-broadcast-api) can publish this stream with WebRTC, HLS, or DASH.
...
For detailed implementation steps and code examples, see the [Developing Plugins](./developing.md) guide.
