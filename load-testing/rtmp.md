...
```mermaid
flowchart LR
    Script[rtmp_publisher.sh] -->|spawn N ffmpeg processes| F1[FFmpeg stream 1]
    Script --> F2[FFmpeg stream 2]
    Script --> FN[FFmpeg stream N]
    F1 & F2 & FN -->|RTMP publish| AMS[Ant Media Server]
    AMS --> Dashboard[Monitor via Dashboard]
```
