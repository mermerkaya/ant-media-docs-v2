...
description: With Ant Media Server WebRTC Samples, you may play and experience through our pre-made examples and see Ant Media Server in action.
...
:::info
In these sample examples, we use `live` as the application, but you can use any different application as well for your testing. You can also create your own application as well.
:::
...
```mermaid
graph TD
    Server["Ant Media Server<br/>https://domain:5443"] --> Pub["Publishing Samples"]
    Server --> Play["Playback Samples"]
    Server --> Conf["Conference / P2P"]
    Server --> Tools["Test Tools"]

    Pub --> WP["WebRTC Publish<br/>/live/index.html"]
    Pub --> AP["Audio Publish<br/>/live/audio_publish.html"]
    Pub --> WB["Whiteboard Publish<br/>/live/canvas-publish.html"]
    Pub --> TS["Publish with Timestamp<br/>/live/publish_with_timestamp.html"]
    Pub --> VBG["Virtual Background<br/>/live/publish_webrtc_virtual_background.html"]
    Pub --> DAR["DeepAR Effects<br/>/live/publish_webrtc_deep_ar_effects.html"]
    Pub --> WHIP["WHIP Publish<br/>/live/whip.html"]

    Play --> WRPlay["WebRTC Play<br/>/live/player.html"]
    Play --> Audio["Audio Play<br/>/live/audio_player.html"]
    Play --> HLS["HLS Play<br/>/live/play.html?playOrder=hls"]
    Play --> MT["Multitrack Player<br/>/live/multitrackplayer.html"]
    Play --> WBP["Whiteboard Play<br/>/live/canvas-player.html"]
    Play --> TPlay["Player with Timestamp<br/>/live/player_with_timestamp.html"]
    Play --> MTM["Merge Streams<br/>/live/multitrack-play.html"]

    Conf --> CC["Conference<br/>/live/conference.html"]
    Conf --> P2P["Peer-to-Peer<br/>/live/peer.html"]
    Conf --> DC["DataChannel<br/>/live/datachannel.html"]

    Tools --> TT["WebRTC Test Tool<br/>/live/webrtc-test-tool.html"]
```
