...
```mermaid
graph TD
    Admin["ADMIN User<br/>(system scope)"]
    User["USER<br/>(app scope)"]
    ReadOnly["READ-ONLY<br/>(app scope)"]

    Admin -->|can do| CRUD["Create / Read / Update / Delete<br/>All Applications & Services"]
    User -->|can do| AppActions["Manage assigned applications<br/>Cannot see other apps"]
    ReadOnly -->|can do| ViewOnly["View dashboard data<br/>Cannot create/modify anything"]

    Admin -.->|scope| System["system"]
    User -.->|scope| AppScope["LiveApp / WebRTCAppEE / live"]
    ReadOnly -.->|scope| AppScope2["LiveApp / WebRTCAppEE / live"]
```
...
### Multi-App Access Example

```mermaid
graph LR
    U["User: abc@antmedia.io"]
    U -->|ADMIN| LiveApp
    U -->|USER| WebRTCAppEE
    U -->|READ_ONLY| live
```
