...
description: Ant Media Server Dashboard Features — explore the web panel, live streams, VoD, settings, and security controls.
...
```mermaid
graph TD
    Login["Login :5080 or :5443"] --> Dashboard

    Dashboard --> Applications
    Dashboard --> Settings

    Applications --> LiveStreams["Live Streams Tab"]
    Applications --> VoD["VoD Tab"]
    Applications --> AppSettings["Settings Tab"]

    LiveStreams --> Actions["Action Menu"]
    Actions --> MP4["Enable MP4/WebM Recording"]
    Actions --> RTMP_EP["RTMP Endpoints (Simulcast)"]
    Actions --> CopyURL["Copy Publish URL"]
    Actions --> EmbedCode["Copy Embed Code"]
    Actions --> PlayWebRTC["Play with WebRTC"]
    Actions --> PlayHLS["Play with HLS"]

    AppSettings --> Codecs["WebRTC Codec Support"]
    AppSettings --> ABR["Adaptive Bitrate"]
    AppSettings --> HLS["HLS Streaming"]
    AppSettings --> Advanced["Advanced Settings"]
    AppSettings --> DataChan["WebRTC Data Channel"]
    AppSettings --> Recording["Stream Recording"]
    AppSettings --> StreamSec["Stream Security"]
    AppSettings --> RestSec["REST API Security"]
```
...
From a local installation of Ant Media Server, here is a real example of a publish URL from a live stream under the LiveApp application.
...
Here is an example embed code taken from a local installation:
