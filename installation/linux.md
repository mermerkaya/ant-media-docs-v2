...
description: Installing Ant Media Server on Linux — Ubuntu, CentOS, Rocky Linux, Alma Linux.
...
Ant Media can be installed on Linux, specifically Ubuntu (20.04, 22.04, and 24.04), CentOS 9, Rocky Linux 9 and Alma Linux 9. It is compatible with both the x86-64 and Arm64 architectures.
...
flowchart TD
    ...
    B -->|Automatic| C[Download install script]
    B -->|Manual| D[Download AMS ZIP]

    C --> E{Edition}
    E -->|Enterprise| F["./install_ant-media-server.sh -l license-key"]
    E -->|Community| G["./install_ant-media-server.sh"]

    D --> H[Download install script]
    H --> I["./install_ant-media-server.sh -i AMS.zip"]
    ...
    J --> K{SSL needed?}
    K -->|Yes| L["./enable_ssl.sh -d domain.com"]
    K -->|No| M[Access :5080]
    L --> N[Access :5443]
```
...
For detailed information about SSL, follow [SSL Setup](/installation/ssl).
...
If an SSL certificate is installed, the server can be reached at `https://domain-name:5443`

## Docker Installation

Please visit for more information [Docker and Docker Compose](/installation/docker).

## Cluster Installation

Cluster installation is an advanced topic and it has its own page. Please visit [Clustering & Scaling](/scale/cluster-install).
...
|------|----------|---------|
| 80 | TCP | SSL challenge (Let's Encrypt) |
| 5080 | TCP | HTTP Web Panel |
| 5443 | TCP | HTTPS Web Panel & WebRTC |
| 4200 | UDP | SRT Ingest |
| 1935 | TCP | RTMP Ingest |
| 50000-60000 | UDP | WebRTC Media (v2.4.3+) |
| 5000 | TCP | Cluster internal communication (not public) |
