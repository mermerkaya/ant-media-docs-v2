...
description: AMS Docker Compose Installation — manage Ant Media Server with docker-compose.
...
```mermaid
flowchart TD
    A[Start] --> B[Download docker-compose.yml + Dockerfile]
    B --> C[Download AMS ZIP file]
    C --> D{Edition?}
    D -->|Enterprise| E["docker-compose build --build-arg AntMediaServer=ams-enterprise.zip"]
    D -->|Community| F["docker-compose build --build-arg AntMediaServer=ams-community.zip"]
    E --> G["docker-compose up -d"]
    F --> G
    G --> H["Access :5080 dashboard"]
```
...
If you would like to mount an existing volume, simply uncomment the below lines in the original yml file.
