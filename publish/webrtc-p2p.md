...
description: Implement WebRTC peer-to-peer communication with Ant Media Server JavaScript SDK.
...
## P2P vs 1-to-Many Comparison

```mermaid
graph TD
    subgraph P2P["Peer-to-Peer Mode"]
        A["Peer A"] <-->|direct WebRTC| B["Peer B"]
        A -.->|signaling only| AMS_P2P["AMS (signaling)"]
        B -.->|signaling only| AMS_P2P
    end

    subgraph OneToMany["1-to-Many Mode"]
        PUB["Publisher"] -->|stream| AMS_1N["AMS (media relay)"]
        AMS_1N -->|stream| PLAY1["Player 1"]
        AMS_1N -->|stream| PLAY2["Player 2"]
        AMS_1N -->|stream| PLAY3["Player N"]
    end
```
