...
description: Use WebRTC Data Channel with Ant Media Server for sending arbitrary data alongside video and audio streams.
...
```mermaid
graph TD
    subgraph Data Channel Delivery Options
        P["Publisher"]
        PL1["Player 1"]
        PL2["Player 2"]
        AMS["Ant Media Server"]

        P -->|publish stream + data| AMS

        AMS -->|"Nobody: data NOT forwarded"| N["(no delivery)"]
        AMS -->|"Only publisher: data from players → publisher only"| P
        AMS -->|"Publisher and all players: data to all"| PL1
        AMS -->|"Publisher and all players: data to all"| PL2
    end
```
...
Go to `https://your-domain-name:5443/live/datachannel.html` to use the data channel sample.
