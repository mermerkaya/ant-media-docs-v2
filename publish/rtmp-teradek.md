---
title: Publish with Teradek (RTMP)
description: Publish RTMP live stream using Teradek Vidiu Pro to Ant Media Server.
keywords: [Teradek RTMP, Ant Media Server Documentation, Ant Media Server Tutorials]
sidebar_position: 8
---

# Publish RTMP Stream Using Teradek Vidiu Pro
...
## Teradek Publishing Flow

```mermaid
graph LR
    HDMI["HDMI Source<br/>(Camera / Video)"] --> Teradek["Teradek Vidiu Pro<br/>(Hardware Encoder)"]
    Teradek -->|RTMP :1935| AMS["Ant Media Server"]
    AMS -->|WebRTC / HLS| Players["Players"]

    subgraph Setup Steps
        S1["1. Power on Teradek"] --> S2["2. Connect to VidiU WiFi"]
        S2 --> S3["3. Open 172.16.1.1 in browser"]
        S3 --> S4["4. Configure WiFi client mode"]
        S4 --> S5["5. Set RTMP URL + Stream ID"]
        S5 --> S6["6. Press Start/Stop button"]
    end
```
