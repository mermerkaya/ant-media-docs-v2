---
title: SRT Ingest with Wirecast
description: Publish SRT streams to Ant Media Server using Wirecast without requiring a streamId.
keywords: [SRT Wirecast, Ant Media Server Documentation, Ant Media Server Tutorials]
sidebar_position: 14
---

# SRT Ingest with Wirecast
...
## Key Points About Wirecast SRT

```mermaid
graph TD
    A["Wirecast (SRT without streamId support)"]
    A --> B["Ant Media Server v2.8.0+<br/>supports SRT without streamId"]
    B --> C["Encoder IP → used as streamId"]
    C --> D["Published to LiveApp by default"]
    D --> E["View stream in AMS Management Panel"]
```
...
- You can choose the source type based on your use case. For this demonstration, add a VoD as **Media File**:
...
:::info
It's important to set the Profile type to **Baseline** for WebRTC playback.
:::

## Starting the SRT Stream

Now that everything is set, proceed to push the SRT stream.
...
- Go to **Output >> Start/Stop Broadcasting >> Start SRT** to start pushing the SRT stream to your Ant Media Server.

  ![](@site/static/img/publish-live-stream/srt/start-stop.png)

- Navigate to the LiveApp application of your Ant Media Server Management Panel and you can see the SRT stream.
