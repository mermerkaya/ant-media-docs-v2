---
title: Publish with OBS (RTMP)
description: Publish RTMP live stream using OBS (Open Broadcaster Software) to Ant Media Server.
keywords: [OBS RTMP, Ant Media Server Documentation, Ant Media Server Tutorials]
sidebar_position: 6
---
...
```mermaid
graph LR
    Camera["Camera / Screen<br/>(OBS Source)"] --> OBS["OBS Encoder<br/>(H.264 CBR)"]
    OBS -->|RTMP :1935| AMS["Ant Media Server"]
    AMS -->|WebRTC| Players["Players<br/>(< 1s latency)"]
    AMS -->|HLS| HLS["HLS Players<br/>(8-12s latency)"]
```
