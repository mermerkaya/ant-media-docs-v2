...
```mermaid
flowchart LR
    P[Publisher] -->|stream| AMS[Ant Media Server]
    AMS -->|CPIX key request| KMS[Key Management Server]
    KMS -->|encryption keys| AMS
    AMS -->|encrypted HLS/DASH| CDN[CDN / Edge]
    CDN -->|DRM license request| LS[License Server]
    LS -->|license| Player[Authorized Player]
    CDN -->|encrypted stream| Player
```
