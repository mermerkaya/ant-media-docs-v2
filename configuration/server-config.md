...
description: Configure Ant Media Server through configuration file or management console — server-level settings.
...
```mermaid
graph TD
    AMS["Ant Media Server"] --> ServerConf["Server Configuration<br/>&lt;AMS_DIR&gt;/conf/red5.properties"]
    AMS --> AppConf["Application Configuration<br/>/usr/local/antmedia/webapps/{AppName}/WEB-INF/red5-web.properties"]

    ServerConf --> General["General Settings<br/>(name, license, CPU, RAM)"]
    ServerConf --> HTTP["HTTP Settings<br/>(port, threads)"]
    ServerConf --> RTMP["RTMP Settings<br/>(port, buffers, timeouts)"]
    ServerConf --> RTMPS["RTMPS Settings<br/>(SSL RTMP)"]
    ServerConf --> RTMPT["RTMPT Settings<br/>(RTMP over HTTP)"]
    ServerConf --> JMX["JMX Settings<br/>(monitoring)"]
    ServerConf --> Cluster["Cluster DB Settings<br/>(MongoDB)"]
```
