---
title: Video Codecs
description: Simplified understanding of H.264, VP8, and H.265 codecs used with Ant Media Server. How to enable H.264, VP8, and H.265.
...
```mermaid
graph TD
    subgraph WebRTC["WebRTC (Ultra-low latency)"]
        H264W["H.264 ✓"]
        VP8W["VP8 ✓"]
        H265W["H.265 ✗ (not supported yet)"]
    end

    subgraph HLS["HLS / LL-HLS"]
        H264H["H.264 ✓"]
        VP8H["VP8 ✗"]
        H265H["H.265 ✓"]
    end

    subgraph DASH["DASH / CMAF"]
        H264D["H.264 ✓"]
        VP8D["VP8 ✗"]
        H265D["H.265 ✓"]
    end
    ...
    subgraph Recording["Recording"]
        H264R["MP4 → H.264 required"]
        VP8R["WebM → VP8 required"]
    end
```
...
```mermaid
flowchart TD
    A[Choose codec configuration] --> B{WebRTC playback needed?}
    B -->|Yes| C{Browser compatibility important?}
    B -->|No, HLS/DASH only| D[Enable H.265 for best quality]
    C -->|Yes| E[Enable H.264]
    C -->|Some browsers| F[Enable H.264 + VP8]
    E --> G{Recording needed?}
    F --> G
    G -->|MP4| H[H.264 required — already enabled]
    G -->|WebM| I[Enable VP8]
    G -->|No| J[Done]
```
