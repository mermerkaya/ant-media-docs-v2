...
description: Measure latency or the delay for a stream to be transmitted from one point to another using TimeServer, OCR, AWS Rekognition or Google's Vision API.
...
```mermaid
sequenceDiagram
    participant Publisher as Publisher<br/>(publish_with_timestamp.html)
    participant AMS as Ant Media Server
    participant Player as Player<br/>(player_with_timestamp.html)
    participant OCR as OCR API<br/>(AWS Rekognition / Google Vision)

    Publisher->>Publisher: Draw current timestamp on canvas
    Publisher->>AMS: Stream with timestamp overlay
    AMS->>Player: Stream delivered
    Player->>Player: Draw play timestamp on canvas
    Player->>OCR: Send frame for text recognition
    OCR-->>Player: Extracted publish timestamp
    Player->>Player: Calculate latency =<br/>play_time - publish_time
    Player->>Player: Display E2E latency
```
