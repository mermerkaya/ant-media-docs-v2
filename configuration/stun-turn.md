...
description: Configure custom STUN and TURN servers for Ant Media Server and client SDKs for reliable WebRTC connectivity.
...
sequenceDiagram
    ...
    Publisher->>STUN: Get public IP (STUN request)
    STUN-->>Publisher: Public IP + Port (ICE candidate)
    Publisher->>AMS: Offer SDP + ICE candidates
    AMS-->>Publisher: Answer SDP + ICE candidates
    Publisher->>AMS: ICE connectivity check
    AMS-->>Publisher: ICE connected
    Note over Publisher,AMS: If direct fails, TURN relay is used
    Publisher->>STUN: TURN allocation (if NAT blocked)
    STUN-->>Publisher: Relay address
    Publisher->>AMS: Relay ICE candidate
```
...
If you are using Ant Media Server Embedded Web Player which is found in this [Github Repo](https://github.com/ant-media/Web-Player) to play your streams you need to pass `iceServers` as String to the WebPlayer constructor.
