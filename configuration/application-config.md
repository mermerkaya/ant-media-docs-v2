...
```mermaid
graph TD
    Config["AMS Application Configuration"] --> WebPanel["Web Management Panel"]
    Config --> File["red5-web.properties File"]
    Config --> API["REST API"]

    WebPanel --> Basic["Basic Settings<br/>(most common)"]
    WebPanel --> Advanced["Advanced Settings<br/>(all settings)"]

    File --> Direct["Direct file edit<br/>/usr/local/antmedia/webapps/{App}/WEB-INF/red5-web.properties"]

    API --> Get["GET /rest/v2/{app} — get settings"]
    API --> Put["PUT /rest/v2/applications/settings/{app} — change settings"]
```
